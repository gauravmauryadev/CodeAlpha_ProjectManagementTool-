            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 gap-3 sm:gap-0">
              <div className="flex items-center gap-3 w-full sm:w-auto min-w-0">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-semibold text-slate-800 dark:text-slate-200 truncate">@{githubStatus.username}</p>
                  <a href={`https://github.com/${githubStatus.username}`} target="_blank" rel="noopener noreferrer" className="text-xs text-indigo-500 hover:text-indigo-600 flex items-center gap-1 w-max">
                    View Profile <ExternalLink className="w-3 h-3" />
                  </a>
                </div>
              </div>
              <button onClick={handleGithubDisconnect} className="w-full sm:w-auto px-4 py-2 rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-500 text-xs font-semibold transition-colors flex items-center justify-center gap-1.5 flex-shrink-0 cursor-pointer">
