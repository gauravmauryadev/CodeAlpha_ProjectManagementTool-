...
import { commentApi, projectApi, authApi, githubApi, discordApi } from "@/lib/api";
...
export default function BoardPage() {
  ...
  // Collaboration state
  const [activeTab, setActiveTab] = useState<"board" | "wiki" | "activity">("board");
    ...
    <AppShell>
      <div className="h-screen flex flex-col overflow-hidden">
        ...
        <div className="px-4 md:px-8 py-4 md:py-5 border-b border-slate-200 dark:border-white/5 flex flex-col md:flex-row md:items-center justify-between gap-4 flex-shrink-0 bg-white/40 dark:bg-[#0E0A22]/40 backdrop-blur-xl">
          <div className="flex items-center gap-3 md:gap-4 mt-2 md:mt-0">
            <Tooltip content="Back to dashboard" position="right">
              <button
                ...
                className="w-9 h-9 rounded-xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 hover:bg-slate-50 dark:hover:bg-white/10 hover:scale-105 active:scale-95 flex items-center justify-center transition-all duration-200 shadow-sm cursor-pointer hover:border-slate-300 dark:hover:border-white/20"
              >
          ...
          <div className="flex items-center gap-2 md:gap-3 pr-0 md:pr-[130px] flex-wrap md:flex-nowrap">
            {/* Members avatars */}
            <div className="flex -space-x-2 mr-2">
              {project?.members?.slice(0, 5).map((m) => (
                <div key={typeof m === "object" ? m._id : m} className={cn("w-8 h-8 rounded-full ring-2 ring-[#0E0A22] flex items-center justify-center text-[10px] font-bold text-white", getAvatarColor(typeof m === "object" ? m.name : "U"))}>
            ...
            <Tooltip content="Project settings" position="bottom">
              <button
                ...
                className="w-10 h-10 rounded-xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 hover:bg-slate-50 dark:hover:bg-white/10 hover:rotate-45 active:scale-95 flex items-center justify-center transition-all duration-200 shadow-sm cursor-pointer hover:border-slate-300 dark:hover:border-white/20"
              >
                <Settings className="w-4.5 h-4.5 text-slate-500 dark:text-slate-200" />
              </button>
            </Tooltip>

            <Tooltip content="Team Chat & Calls" position="bottom">
              <button
                onClick={() => setShowChat(!showChat)}
                className={cn(
                  "px-3 md:px-4 py-2 rounded-xl border text-xs md:text-sm font-medium flex items-center gap-1 md:gap-2 transition-all duration-200 shadow-sm cursor-pointer",
                  showChat 
                    ? "bg-indigo-600 border-indigo-650 text-white hover:bg-indigo-500 hover:-translate-y-0.5" 
                    : "bg-white dark:bg-white/5 border-slate-200 dark:border-white/10 hover:bg-slate-50 dark:hover:bg-white/10 hover:border-slate-300 dark:hover:border-white/20 hover:-translate-y-0.5 active:scale-97 text-slate-600 dark:text-slate-200"
                )}
              >
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="flex-1 flex flex-col overflow-hidden min-w-0">
            ...
            <div className="px-4 md:px-8 py-3 border-b border-slate-200 dark:border-white/5 bg-slate-100/50 dark:bg-[#0e0a22]/20 flex items-center gap-2 flex-shrink-0 overflow-x-auto no-scrollbar">
              <button
                onClick={() => setActiveTab("board")}
                className={cn(
                  "px-4 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5",
                  activeTab === "board" ? "bg-indigo-600 text-white shadow-lg shadow-indigo-600/30" : "text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200"
                )}
              >
                📋 Kanban Board
              </button>
              <button
                ...
                className={cn(
                  "px-4 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5",
                  activeTab === "wiki" ? "bg-indigo-600 text-white shadow-lg shadow-indigo-600/30" : "text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200"
                )}
              >
                📝 Project Wiki
              </button>
              <button
                ...
                className={cn(
                  "px-4 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5",
                  activeTab === "activity" ? "bg-indigo-600 text-white shadow-lg shadow-indigo-600/30" : "text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200"
                )}
              >
                <GithubIcon className="w-3.5 h-3.5" /> Activity
              </button>
            </div>
                  ...
                  <div className="flex gap-4 md:gap-5 h-full w-max">
                    {columns.map((col) => {
                      ...
                      return (
                        ...
                        >
                        {/* Column Header */}
                        <div className={cn("flex items-center justify-between px-4 py-3 rounded-xl mb-3 border shadow-sm", col.color, col.bg)}>
                        ...
                        <div className="flex-1 space-y-2.5 overflow-y-auto pr-1">
                          {colTasks.map((task) => (
                            <DraggableTask
                              ...
                              className="group p-4 rounded-xl border-2 border-transparent bg-white dark:bg-[#14112c]/45 hover:bg-slate-50 dark:hover:bg-[#1a163a]/60 hover:border-indigo-500/40 cursor-pointer transition-all shadow-sm animate-border-pulse"
                            >
              ...
              <div className="flex-1 overflow-y-auto p-6 lg:p-10">
                <div className="max-w-3xl mx-auto">
                  ...
                  {githubActivity.length === 0 ? (
                    <div className="text-center p-12 rounded-2xl border-2 border-dashed border-slate-200 dark:border-white/10">
                    ...
                    <div className="space-y-4">
                      {githubActivity.map((item, i) => (
                        <a key={`${item.type}-${item.sha || item.number}-${i}`} href={item.url} target="_blank" rel="noopener noreferrer" className="block p-5 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 hover:border-indigo-500/50 hover:shadow-md transition-all group">
                    ...
                    {selectedTask.githubIssueNumber ? (
                      ...
                      project?.githubRepo && (
                        <button
                          ...
                          className="w-full flex items-center justify-center gap-2 p-2.5 rounded-xl border border-dashed border-slate-600 hover:border-indigo-500 hover:bg-indigo-500/10 text-slate-400 hover:text-indigo-400 transition-all text-sm font-semibold disabled:opacity-50"
                        >
                          {isCreatingIssue ? (
      ...
      {showMembers && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="w-full max-w-md bg-[#13102d]/95 backdrop-blur-2xl border-2 border-transparent rounded-2xl p-6 shadow-2xl text-slate-200 animate-in fade-in zoom-in-95 duration-200 animate-border-pulse">
            ...
            <div className="relative mb-5">
              ...
              {searchResults.length > 0 && (
                <div className="absolute z-10 w-[calc(100%-80px)] mt-1 bg-[#1a163a] border border-white/10 rounded-xl shadow-xl overflow-hidden max-h-48 overflow-y-auto">
                  {searchResults.map(u => (
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {project?.members?.map((m) => {
                ...
                return (
                  <div key={typeof m === "object" ? m._id : m} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-white/5">
              ...
              <div className="p-5 rounded-xl border border-white/10 bg-white/5">
                ...
                <div className="space-y-4">
                  ...
                  <button 
                    ...
                    className="w-full px-4 py-2.5 rounded-lg bg-[#5865F2] hover:bg-[#4752C4] disabled:opacity-50 text-white text-sm font-semibold transition-all flex items-center justify-center gap-2"
                  >
