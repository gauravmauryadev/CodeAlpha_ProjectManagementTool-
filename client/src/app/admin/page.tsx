    ...
    <AppShell>
        ...
        <div className="rounded-2xl border-2 border-transparent bg-white dark:bg-[#14112c]/45 shadow-sm overflow-hidden animate-border-pulse overflow-x-auto">
          {tab === "users" ? (
            <table className="w-full">
              <thead>
                <tr className="border-b border-black/5 dark:border-white/5">
                  ...
                  <th className="text-left px-5 py-3.5 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Joined</th>
              ...
              <tbody>
                {filteredUsers.map((u) => (
                  <tr key={u._id} className="border-b border-black/[0.03] dark:border-white/5 hover:bg-white/40 dark:hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-5 py-3.5 text-sm text-slate-400">{formatDate(u.createdAt || "")}</td>
                    <td className="px-5 py-3.5 text-right flex items-center justify-end gap-3">
                      <button onClick={() => setSelectedUser(u)} className="text-slate-400 hover:text-indigo-600 transition-colors cursor-pointer"><Eye className="w-4 h-4" /></button>
                      <button onClick={() => handleDeleteUser(u._id)} className="text-slate-400 hover:text-red-500 transition-colors cursor-pointer"><Trash2 className="w-4 h-4" /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-black/5 dark:border-white/5">
                  <th className="text-left px-5 py-3.5 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Project</th>
                  ...
                  <th className="text-left px-5 py-3.5 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Members</th>
                  <th className="text-left px-5 py-3.5 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Created</th>
                  <th className="text-right px-5 py-3.5 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Actions</th>
                </tr>
              </thead>
              <tbody>
                {filteredProjects.map((p) => {
                  ...
                  return (
                    <tr key={p._id} className="border-b border-black/[0.03] dark:border-white/5 hover:bg-white/40 dark:hover:bg-white/5 transition-colors">
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold text-white shadow-sm" style={{ background: p.color || "#6366f1" }}>{p.name.charAt(0)}</div>
                          <span className="text-sm font-medium text-slate-800 dark:text-slate-200">{p.name}</span>
                        </div>
                      </td>
                      <td className="px-5 py-3.5 text-sm text-slate-500 dark:text-slate-400">{ownerName}</td>
                      <td className="px-5 py-3.5 text-sm text-slate-500 dark:text-slate-400">{p.members?.length || 0}</td>
                      <td className="px-5 py-3.5 text-sm text-slate-400">{formatDate(p.createdAt)}</td>
                      <td className="px-5 py-3.5 text-right flex items-center justify-end gap-2">
                        <a href={`/board/${p._id}`} className="text-slate-400 hover:text-indigo-650 transition-colors cursor-pointer"><ExternalLink className="w-4 h-4" /></a>
                        <button onClick={() => handleDeleteProject(p._id)} className="text-slate-400 hover:text-red-500 transition-colors cursor-pointer"><Trash2 className="w-4 h-4" /></button>
                      </td>
                    </tr>
                  );
                })}
      ...
      {selectedUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in-up">
          <div className="w-full max-w-md bg-white dark:bg-[#14112c] rounded-2xl border-2 border-transparent shadow-2xl overflow-hidden flex flex-col relative animate-fade-in-up animate-border-pulse">
            <button 
            ...
            <div className="px-6 pb-6 relative">
              <div className="w-20 h-20 rounded-2xl border-4 border-white dark:border-[#14112c] bg-slate-100 dark:bg-[#1a163a] flex items-center justify-center shadow-md absolute -top-10 overflow-hidden">
                {selectedUser.avatar ? (
...
function StatCard({ icon: Icon, label, value, gradient }: { icon: React.ElementType; label: string; value: number; gradient: string }) {
  return (
    <div className="p-5 rounded-2xl border-2 border-transparent bg-white dark:bg-[#14112c]/45 hover:bg-slate-50 dark:hover:bg-[#1a163a]/60 shadow-sm transition-all group animate-border-pulse">
      <div className="flex items-center gap-4">
        <div className={cn("w-11 h-11 rounded-xl bg-gradient-to-br flex items-center justify-center shadow-lg group-hover:scale-105 transition-transform", gradient)}>
